...
import numpy as np
"""
我们基于书本上的关于选址-运输问题改良拉格朗日松弛算例
书目上的题目和我们需要解决的问题有非常相似的地方
但是我们去除代码中数据类型的确定和读取数据的部分
所以该算例存在两个部分:
...
for i in range(car_seattype):
    for j in range(car_type):
        X_1[i][j] = model.addVar(lb= 0, ub= car_seat_Matrix[i][j], vtype=GRB.CONTINUOUS, name="X1[" + str(i+1) + "][" + str(j+1)+"]")
for i in range(car_seattype):
    for j in range(car_type):
        X_2[i][j] = model.addVar(lb= 0, ub= car_seat_Matrix[i][j], vtype=GRB.CONTINUOUS, name="X2[" + str(i+1) + "][" + str(j+1)+"]")
#以下设置multiple类的机器
for j in range(car_type):
    max_jcar_seat = max(car_seat_Matrix[0][j], car_seat_Matrix[1][j], car_seat_Matrix[2][j], car_seat_Matrix[3][j])
    X_3[j]= model.addVar(lb= 0, ub= max_jcar_seat, vtype=GRB.CONTINUOUS, name="X3[" + str(j+1)+"]")
for j in range(car_type):
    max_jcar_seat = max(car_seat_Matrix[0][j], car_seat_Matrix[1][j], car_seat_Matrix[2][j], car_seat_Matrix[3][j])
    X_4[j]= model.addVar(lb= 0, ub= max_jcar_seat, vtype=GRB.CONTINUOUS, name="X4[" + str(j+1)+"]")
#以下设置single类的机器的Y类型变量
for i in range(car_seattype):
    for j in range(car_type):
        Y_1[i][j] = model.addVar(lb = 0,ub=1,vtype=GRB.CONTINUOUS, name="Y1[" + str(i+1) + "][" + str(j+1)+"]")
for i in range(car_seattype):
    for j in range(car_type):
        Y_2[i][j] = model.addVar(lb = 0,ub=1,vtype=GRB.CONTINUOUS, name="Y2[" + str(i+1) + "][" + str(j+1)+"]")
#以下设置multiple类的机器的Y类型变量
for j in range(car_type):
    Y_3[j]= model.addVar(lb = 0,ub=1,vtype=GRB.CONTINUOUS, name="Y3[" + str(j+1)+"]")
for j in range(car_type):
    Y_4[j]= model.addVar(lb = 0,ub=1,vtype=GRB.CONTINUOUS, name="Y4[" + str(j+1)+"]")
#设置时间长度的变量C
for i in range(4):
