...
import numpy as np
"""
载入包之后，具体分析以下的数学建模变量、目标
以及约束之间的关联和联系。首先我们需要建立
...
"""
以上设置完了变量，和变量的上下界限，接下来需要建立三个约束和目标函数
"""
#目标函数，目标函数为最小化C
obj = LinExpr(0)
...
#C_total是四个时间里面最大的那个
model.addGenConstrMax(C_total, [C_group[0], C_group[1], C_group[2], C_group[3]], name="最大的那个机器时间")
...
#最优化运行，并且给出约束性表格
model.write("linar_parallel_production.lp")
model.optimize()
